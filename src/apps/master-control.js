...
export function createMasterControlApp({
  ...
  function renderAuthority(runtime, production, content) {
    ...
    for (const lease of leases) {
      ...
      const agent = runtime.agents.find(item => item.id === lease.agentId);
      content.append(el('article', { class: 'master-object-row utility-row', dataset: { utilityId: 'gummy.utility.agent' } }, [
        tileImage('gummy.utility.agent', 64),
        el('div', {}, [
          el('strong', { text: lease.agentId }),
          el('small', { text: `Runtime ${agent?.runtimeClass || 'unknown'} · locality ${agent?.locality || 'unknown'}` }),
          el('small', { text: `Mold ${lease.moldId} · Lease ${lease.id} · Grant ${grant?.id || 'none'}` })
        ])
      ]));
    }
    if (!leases.length) content.append(el('div', { class: 'empty-state utility-empty' }, [
      tileImage('gummy.utility.agent', 96),
      el('strong', { text: 'No active Agent Runtime' }),
      el('span', { text: 'Leases and Grants are created only after Human approval and Make Production.' })
    ]));
  }
  ...
  function renderContext(runtime, production, content) {
    ...
    if (!envelopes.length) {
      content.append(el('div', { class: 'empty-state utility-empty' }, [
        tileImage('gummy.utility.vision', 96),
        el('strong', { text: 'No Context Envelopes yet' }),
        el('span', { text: 'Inspectable context slices are created only for approved execution nodes.' })
      ]));
      return;
    }
  ...
  function renderEvidence(runtime, production, content) {
    ...
    content.append(el('div', { class: 'utility-section-heading compact' }, [
      tileImage('gummy.utility.deliver', 64),
      el('h3', { text: `Returns (${returns.length})` })
    ]));
