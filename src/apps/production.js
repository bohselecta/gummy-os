...
export function createProductionApp({
  ...
  function renderCanvas(runtime, production, main) {
    ...
    for (const participant of participants) {
      ...
      const card = el('article', {
        ...
        ondragstart: event => event.dataTransfer.setData('application/x-gummy-actor', actor.id)
      }, [
    ...
    if (!participants.length) roster.append(el('div', { class: 'drop-target-empty', text: 'Drop an Actor here or use @mention/search.' }));
    main.append(roster);
  ...
  function renderPlan(runtime, production, main) {
    ...
    if (!plan) {
      main.append(el('div', { class: 'empty-state', text: 'Compile the roster into a visible Actor Plan.' }));
      return;
    }
    ...
    for (const node of plan.nodes) {
      ...
      graph.append(el('article', {
        ...
        draggable: 'true',
        ondragstart: event => event.dataTransfer.setData('application/x-gummy-plan-node', node.id)
      }, [
  ...
  function renderGummies(runtime, production, main) {
    main.append(sectionHeading('Gummy shelf', 'Source, references, results, deliverables, Returns, and Receipts stay inspectable.'));
    ...
    for (const gummyId of production.gummyIds) {
      ...
      shelf.append(el('article', {
        ...
        draggable: 'true',
        ondragstart: event => event.dataTransfer.setData('application/x-gummy-object', gummy.id)
      }, [
