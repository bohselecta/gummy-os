...
export function createReceiptsApp({ store }) {
  ...
  const receipts = [...state.receipts, ...state.productionRuntime.receipts].reverse();
  ...
  for (const receipt of receipts) {
    root.append(el('article', { class: 'receipt' }, [
      ...
      el('small', { text: `${receipt.application || receipt.agentId || 'Gummy OS Runtime'} · ${receipt.actor || receipt.actorId || 'human:hayden'} · ${new Date(receipt.createdAt).toLocaleString()}` }),
      el('p', { text: receipt.detail || receipt.summary || 'Completed.' }),
      el('div', { class: 'tag-row' }, [
        el('span', { class: 'tag', text: receipt.outcome }),
        el('span', { class: 'tag', text: receipt.reversible === false ? 'irreversible' : 'evidence retained' }),
