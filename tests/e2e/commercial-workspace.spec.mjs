...
test('Command Center orients before and after the optional local example', async ({ page }) => {
  ...
  await expect(command.getByText('COMMAND CENTER · LIVING COLLABORATION')).toBeVisible();
  ...
  await expect(command.locator('[data-phase16-step="make-production"]')).toContainText('Complete in this browser');
  ...
  await expect(command.getByText('Create the full local example')).toBeVisible();
