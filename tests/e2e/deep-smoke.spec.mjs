...
test('Human-owned workspace and goal-first Composer survive a cross-browser return visit', async ({ page }) => {
  ...
  const humanNode = composer.locator('[data-ref-kind="actor"][data-ref-id="actor:hayden"]');
  await expect(humanNode).toBeVisible();
  await expect(humanNode).toContainText('Cross Browser Tester');
  ...
  const restoredHumanNode = page.getByTestId('composer-surface').locator('[data-ref-kind="actor"][data-ref-id="actor:hayden"]');
  await expect(restoredHumanNode).toBeVisible();
  await expect(restoredHumanNode).toContainText('Cross Browser Tester');
});
