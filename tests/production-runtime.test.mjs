...
test('migration is deterministic, idempotent, traceable, and preserves legacy state', () => {
  const legacy = {
    snack: { id: 'snack:legacy', name: 'Legacy Hayden', handle: '@legacy' },
    files: [{ id: 'file:legacy', name: 'Keep me', content: 'preserved' }],
    receipts: [{ id: 'receipt:legacy', schema: 'gummy.action-receipt/v0' }]
  };
  const first = migrateState(legacy);
  const second = migrateState(first);
  assert.equal(first.stateVersion, 2);
  assert.equal(first.files[0].content, 'preserved');
  assert.equal(first.receipts[0].id, 'receipt:legacy');
  assert.equal(first.productionRuntime.migrationLog[0].preservesLegacyState, true);
